...
class Agent(context: Context) {
    ...
    companion object {
        ...
        fun get(context: Context): Agent =
            instance ?: synchronized(this) { instance ?: Agent(context).also { instance = it } }
    }
    ...
    fun send(userText: String, onResult: (Outcome) -> Unit) {
        resume()
        ...
        pool.execute {
            ...
            bus.repo.saveMessage(AiMessage(role = "assistant", text = outcome.reply,
                meta = outcome.route))
            onResult(outcome)
        }
    }
    ...
    private fun handle(userText: String): Outcome {
        ...
        if (prefs.cloudReady() && !stopped.get()) {
            ...
            // fall through to the deterministic fallback on any provider failure
            val fallback = fallback(userText, plan)
            return fallback.copy(reply = fallback.reply, error = reply.error, route = "local-fallback")
        }
    ...
    /** Parse the model's JSON tool call and execute the plan as one group. */
    private fun interpretCloud(text: String): Outcome {
        val obj = extractJson(text)
        if (obj == null) {
            // Plain prose answer.
            return Outcome(text.trim(), route = "cloud")
        }
        ...
        val commands = obj.optJSONArray("commands")?.objects()
            ...
            ?: emptyList()
        if (commands.isEmpty()) {
            return Outcome(reply.ifBlank { text.trim() }, route = "cloud")
        }
    ...
    private fun runCommands(
        ...
        // Automatic snapshot before multi-step or destructive work.
        ...
        for ((name, args) in commands) {
            if (stopped.get()) {
                blocked.add("stopped before $name")
                break
            }
            ...
            if (res.ok) {
                actions.add(res.message)
                lastMessage = res.message
            } else {
                blocked.add("$name failed: ${res.message}")
            }
        }
