...
object CompilerV2 {
        ...
        for (upgrade in phase.upgrades) {
            out.add(Requirement(
                ...
                plannedCommand = jsonOf(
                    ...
                    "args" to jsonOf(
                        "habit" to upgrade.habitId.ifBlank { "TODO" },
    ...
    fun weeklyReport(
        ...
        if (stats.consistency30 >= plan.phases[plan.currentPhaseIndex].metrics.minConsistency) {
            sb.append("On track. Next upgrade: ").append(
                when (plan.decision) { UpgradeDecision.UPGRADE.name -> "ready to advance" else -> "evaluate weekly" }
            ).append("\n")
        } else {
    ...
    // Forward compatibility for the future enum generation
    private val UpgradeDecision.name: String get() = this.name
