 ...
 *
 * Every meaningful operation in SuperFlow lives here exactly once. The manual
 * screens call these commands, and the AI tool registry exposes the same list,
 * which is what makes manual/AI parity structural rather than aspirational.
 */
object Capabilities {
    ...
    private fun identityCaps() = listOf(
        ...
        Capability(
            ...
            listOf("id" to "string", "statement" to "string", "lifeArea" to "string", "status" to "string"), Risk.LOW
        ) { c ->
        ...
        Capability(
            "delete_identity", "Delete an identity", listOf("id" to "string"), Risk.HIGH, destructive = true
        ) { c ->
            val old = c.repo.identity(c.str("id")) ?: return@Capability CommandResult.fail("Identity not found")
        ...
        Capability(
            ...
            listOf("title" to "string", "why" to "string", "identityId" to "string", "outcomeMetric" to "string"),
            Risk.LOW
        ) { c ->
        ...
        Capability(
            ...
            listOf("id" to "string", "title" to "string", "why" to "string", "status" to "string"), Risk.LOW
        ) { c ->
            ...
            val updated = old.copy(
                ...
                status = if (c.str("status").isBlank()) old.status else GoalStatus.valueOf(c.str("status").uppercase())
            )
        ...
        Capability(
            "delete_goal", "Delete a goal", listOf("id" to "string"), Risk.HIGH, destructive = true
        ) { c ->
            val old = c.repo.goal(c.str("id")) ?: return@Capability CommandResult.fail("Goal not found")
        ...
        Capability(
            "delete_system", "Delete a system", listOf("id" to "string"), Risk.HIGH, destructive = true
        ) { c ->
            val old = c.repo.system(c.str("id")) ?: return@Capability CommandResult.fail("System not found")
    ...
    private fun habitCaps() = listOf(
        Capability(
            ...
            listOf(
                "title" to "string", "tinyStart" to "string", "cueTime" to "HH:mm", "cuePlace" to "string",
                "anchorText" to "string", "mode" to "BUILD|REDUCE", "trackType" to "BINARY|COUNT|DURATION",
                "targetCount" to "int", "unit" to "string", "systemId" to "string", "identityId" to "string",
                "days" to "e.g. mon,tue or daily or weekdays", "reminder" to "bool", "protected" to "bool"
            ), Risk.LOW
        ) { c ->
            val title = c.str("title").trim()
            ...
            val h = Habit(
                ...
                identityId = c.strOrNull("identityId") ?: c.repo.identities().firstOrNull()?.id,
                title = title,
                mode = runCatching { HabitMode.valueOf(c.str("mode", "BUILD").uppercase()) }.getOrDefault(HabitMode.BUILD),
                ...
                standardVersion = c.str("standardVersion", title), stretchVersion = c.str("stretchVersion"),
                ...
                orderIndex = c.repo.habits(true).size
            )
            c.repo.saveHabit(h)
        ...
        Capability(
            ...
            var updated = old
            // Field/value form, used by conversational edits.
            ...
            // Direct named arguments, used by forms and Blueprint execution.
        ...
        Capability(
            "archive_habit", "Archive a habit without losing its history",
            listOf("habit" to "id or title"), Risk.MEDIUM
        ) { c ->
            val old = resolveHabit(c) ?: return@Capability CommandResult.fail("Habit not found")
        ...
        Capability(
            "restore_habit", "Return an archived habit to active",
            listOf("habit" to "id or title"), Risk.LOW
        ) { c ->
            val old = resolveHabit(c) ?: return@Capability CommandResult.fail("Habit not found")
        ...
        Capability(
            "delete_habit", "Permanently delete a habit and its check-ins",
            listOf("habit" to "id or title"), Risk.HIGH, destructive = true
        ) { c ->
            val old = resolveHabit(c) ?: return@Capability CommandResult.fail("Habit not found")
            val rows = JSONArray()
        ...
        Capability(
            "reorder_habit", "Move a habit up or down the Today timeline",
            listOf("habit" to "id or title", "direction" to "up|down"), Risk.LOW
        ) { c ->
            ...
            val to = if (c.str("direction", "up").lowercase() == "down") idx + 1 else idx - 1
        ...
        Capability(
            "add_obstacle_plan", "Add an if-then Obstacle Plan to a habit",
            listOf("habit" to "id or title", "ifText" to "string", "thenText" to "string"), Risk.LOW
        ) { c ->
        ...
        Capability(
            "delete_obstacle_plan", "Remove an Obstacle Plan", listOf("id" to "string"), Risk.MEDIUM,
            destructive = true
        ) { c ->
    ...
    private fun checkInCaps() = listOf(
        Capability(
            "check_in", "Record a completion at Tiny, Minimum, Standard or Stretch",
            listOf("habit" to "id or title", "level" to "TINY|MINIMUM|STANDARD|STRETCH",
                "amount" to "number", "date" to "yyyy-MM-dd", "note" to "string"), Risk.LOW
        ) { c ->
            val h = resolveHabit(c) ?: return@Capability CommandResult.fail("Habit not found")
            val date = c.date()
            ...
            okResult("Marked ${level.label.lowercase()} for \"${h.title}\"", jsonOf("id" to ci.id), id)
        },

        Capability(
            "skip_habit", "Intentionally skip a habit today, without shame",
            listOf("habit" to "id or title", "date" to "yyyy-MM-dd", "note" to "string"), Risk.LOW
        ) { c ->
            val h = resolveHabit(c) ?: return@Capability CommandResult.fail("Habit not found")
            val date = c.date()
            val prev = c.repo.checkIn(h.id, date)
            val ci = CheckIn(habitId = h.id, date = date, result = CheckInResult.SKIPPED, note = c.str("note"))
            c.repo.saveCheckIn(ci)
            val undo = if (prev == null) jsonOf("kind" to "clearCheckIn", "habitId" to h.id, "date" to date)
            else undoRestore("checkin", Serial.of(prev))
            val id = c.bus.record(c.actor, "skip_habit", "Skipped \"${h.title}\" intentionally",
                Serial.of(ci), undo, c.groupId)
            okResult("Skipped \"${h.title}\" - that is a choice, not a failure", null, id)
        },

        Capability(
            "mark_missed", "Record that an opportunity passed",
            listOf("habit" to "id or title", "date" to "yyyy-MM-dd"), Risk.LOW
        ) { c ->
            val h = resolveHabit(c) ?: return@Capability CommandResult.fail("Habit not found")
            val date = c.date()
            val prev = c.repo.checkIn(h.id, date)
        ...
        Capability(
            "clear_check_in", "Remove today's check-in for a habit",
            listOf("habit" to "id or title", "date" to "yyyy-MM-dd"), Risk.MEDIUM
        ) { c ->
            val h = resolveHabit(c) ?: return@Capability CommandResult.fail("Habit not found")
            val date = c.date()
            val prev = c.repo.checkIn(h.id, date) ?: return@Capability CommandResult.fail("No check-in to clear")
        ...
        Capability(
            "log_energy", "Log energy at a checkpoint (1-5)",
            listOf("energy" to "1-5", "checkpoint" to "MORNING|MIDDAY|EVENING", "date" to "yyyy-MM-dd",
                "note" to "string"), Risk.LOW
        ) { c ->
            ...
            c.repo.saveEnergy(e)
            val id = c.bus.record(c.actor, "log_energy", "${cp.label} energy: $level/5", Serial.of(e), null, c.groupId)
            okResult("Energy logged", null, id)
        },

        Capability(
            "start_recovery", "Create a compassionate return plan after a miss",
            listOf("habit" to "id or title"), Risk.LOW
        ) { c ->
            val h = resolveHabit(c) ?: return@Capability CommandResult.fail("Habit not found")
    ...
    private fun focusCaps() = listOf(
        Capability(
            "set_daily_focus", "Set up to three Daily Focus actions",
            listOf("items" to "array of strings", "date" to "yyyy-MM-dd"), Risk.MEDIUM
        ) { c ->
            val date = c.date()
        ...
        Capability(
            "add_focus_item", "Add one action to today's Daily Focus",
            listOf("title" to "string", "date" to "yyyy-MM-dd"), Risk.LOW
        ) { c ->
            val date = c.date()
            val title = c.str("title").trim()
        ...
        Capability(
            "complete_focus_item", "Tick off a Daily Focus action",
            listOf("id" to "string", "title" to "string", "done" to "bool"), Risk.LOW
        ) { c ->
            val date = c.date()
            ...
            val target = items.firstOrNull { it.id == c.str("id") }
                ...
                ?: items.firstOrNull { it.title.lowercase().contains(c.str("title").lowercase().trim()) &&
            ...
            okResult(if (updated.done) "Nice - that is a vote for who you are becoming" else "Reopened", null, id)
        },

        Capability(
            "clear_daily_focus", "Clear the Daily Focus list for a day",
            listOf("date" to "yyyy-MM-dd"), Risk.MEDIUM, destructive = true
        ) { c ->
            val date = c.date()
            val prev = c.repo.focusFor(date)
        ...
        Capability(
            "plan_tomorrow", "Draft tomorrow's Daily Focus from scheduled habits",
            listOf(), Risk.MEDIUM
        ) { c ->
    ...
    private fun designCaps() = listOf(
        Capability(
            "add_scorecard_entry", "Record a current routine as helpful, neutral or unhelpful",
            listOf("routine" to "string", "verdict" to "-1|0|1", "note" to "string"), Risk.LOW
        ) { c ->
        ...
        Capability(
            "delete_scorecard_entry", "Remove a scorecard row", listOf("id" to "string"), Risk.MEDIUM,
            destructive = true
        ) { c ->
        ...
        Capability(
            "create_flow", "Create a chain of anchored habits",
            listOf("title" to "string", "anchor" to "string", "steps" to "array of strings"), Risk.LOW
        ) { c ->
            val title = c.str("title").trim()
            ...
            if (steps != null) for (i in 0 until steps.length()) {
                ...
                val habit = c.repo.findHabit(t)
                c.repo.saveFlowStep(FlowStep(flowId = f.id, habitId = habit?.id, title = t, orderIndex = i))
            }
        ...
        Capability(
            "add_flow_step", "Append a step to a flow",
            listOf("flowId" to "string", "title" to "string", "existing" to "bool"), Risk.LOW
        ) { c ->
        ...
        Capability(
            "delete_flow", "Delete a flow and its steps", listOf("flowId" to "string"), Risk.HIGH,
            destructive = true
        ) { c ->
            val flow = c.repo.flows().firstOrNull { it.id == c.str("flowId") }
                ?: return@Capability CommandResult.fail("Flow not found")
        ...
        Capability(
            "enter_minimum_mode", "Drop every non-protected habit to its Minimum for today",
            listOf("date" to "yyyy-MM-dd"), Risk.MEDIUM
        ) { c ->
            val date = c.date()
        ...
        Capability(
            "run_checkpoint", "Run the morning, midday or evening checkpoint",
            listOf("checkpoint" to "MORNING|MIDDAY|EVENING"), Risk.LOW
        ) { c ->
            val cp = runCatching { Checkpoint.valueOf(c.str("checkpoint", "MORNING").uppercase()) }
                .getOrDefault(Checkpoint.MORNING)
            val date = Dates.today()
            val habits = c.repo.habitsForDay(date)
            val done = c.repo.checkInsFor(date).count { it.result == CheckInResult.DONE || it.result == CheckInResult.RESISTED }
            val focus = c.repo.focusFor(date)
            val text = when (cp) {
                Checkpoint.MORNING -> "Morning checkpoint. ${habits.size} habits scheduled. " +
                        if (focus.isEmpty()) "Pick up to three Daily Focus actions." else
                            "Focus: ${focus.joinToString(", ") { it.title }}."
    ...
    private fun reviewCaps() = listOf(
        Capability(
            "create_review", "Save a weekly, monthly or quarterly review",
            listOf("kind" to "WEEKLY|MONTHLY|QUARTERLY", "whatWorked" to "string", "whatDidnt" to "string",
                "systemChange" to "string", "identityEvidence" to "string"), Risk.LOW
        ) { c ->
            ...
            c.repo.saveReview(r)
            val id = c.bus.record(c.actor, "create_review", "${kind.name.lowercase().replaceFirstChar { it.uppercase() }} review saved",
                Serial.of(r), undoDelete("review", r.id), c.groupId)
            okResult("Review saved", jsonOf("id" to r.id), id)
        },

        Capability(
            "delete_review", "Delete a review", listOf("id" to "string"), Risk.HIGH, destructive = true
        ) { c ->
    ...
    private fun queryCaps() = listOf(
        Capability("list_habits", "List habits with today's status", listOf("date" to "yyyy-MM-dd"), Risk.LOW) { c ->
            val date = c.date()
            ...
            val text = habits.joinToString("\n") { h ->
                ...
                val mark = when {
                    ...
                    ci.result == CheckInResult.DONE || ci.result == CheckInResult.RESISTED -> ci.level.label.lowercase()
        ...
        Capability("search", "Search habits, goals, identities and systems",
            ...
            c.repo.identities().filter { it.statement.lowercase().contains(q) }.forEach { hits.add("Identity: ${it.statement}") }
            c.repo.goals().filter { it.title.lowercase().contains(q) }.forEach { hits.add("Goal: ${it.title}") }
            c.repo.systems().filter { it.title.lowercase().contains(q) }.forEach { hits.add("System: ${it.title}") }
            c.repo.habits(true).filter { it.title.lowercase().contains(q) }.forEach { hits.add("Habit: ${it.title}") }
    ...
    private fun dataCaps() = listOf(
        ...
        Capability("clear_ai_history", "Clear the AI conversation and activity trail", listOf(),
    ...
    private fun applyField(h: Habit, field: String, value: String): Habit? = when (field.lowercase()) {
        ...
        "tracktype", "track" -> runCatching { h.copy(trackType = TrackType.valueOf(value.uppercase())) }.getOrDefault(h)
