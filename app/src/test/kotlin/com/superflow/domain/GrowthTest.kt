...
class GrowthTest {
    ...
    @Test
    fun `evaluateWeekly calculates average energy from energy logs in past 7 days`() {
        val today = LocalDate.of(2026, 8, 20)
        val habit = Habit(id = "h1", title = "Meditate")
        val plan = GrowthEngine.generateGrowthPlan(habit)
        val logs = listOf(
            EnergyLog(date = "2026-08-20", checkpoint = Checkpoint.MORNING, energy = 4),
            EnergyLog(date = "2026-08-18", checkpoint = Checkpoint.EVENING, energy = 2),
            EnergyLog(date = "2026-08-10", checkpoint = Checkpoint.MIDDAY, energy = 1) // Outside 7-day window
        )
        val repo = createGrowthTestRepository(habits = listOf(habit), energyLogs = logs)
    ...
    @Test
    fun `evaluateWeekly sets average energy to null when no energy logs exist in past 7 days`() {
        val today = LocalDate.of(2026, 8, 20)
        val habit = Habit(id = "h1", title = "Meditate")
        val plan = GrowthEngine.generateGrowthPlan(habit)
        val logs = listOf(
            EnergyLog(date = "2026-08-10", checkpoint = Checkpoint.MIDDAY, energy = 5) // Outside 7-day window
        )
        val repo = createGrowthTestRepository(habits = listOf(habit), energyLogs = logs)
    ...
    @Test
    fun `evaluateWeekly respects minEnergy requirement for phase upgrade`() {
        val today = LocalDate.of(2026, 8, 20)
        val habit = Habit(id = "h1", title = "Meditate")
        val basePlan = GrowthEngine.generateGrowthPlan(habit)
        // Require minEnergy = 4 in phase 0 metrics and set minWeeksInPhase = 0
        val phase0 = basePlan.phases[0].copy(metrics = PhaseMetrics(minConsistency = 0, minEnergy = 4))
        val plan = basePlan.copy(
            phases = listOf(phase0) + basePlan.phases.drop(1),
            upgradePolicy = basePlan.upgradePolicy.copy(minWeeksInPhase = 0)
        )

        // Average energy is 2.0 (< 4)
        val lowEnergyLogs = listOf(
            EnergyLog(date = "2026-08-20", checkpoint = Checkpoint.MORNING, energy = 2)
        )
        val repoLow = createGrowthTestRepository(habits = listOf(habit), energyLogs = lowEnergyLogs)
        val snapshotLow = GrowthEngine.evaluateWeekly(plan, repoLow, today)
        assertEquals(UpgradeDecision.HOLD, snapshotLow.decision)

        // Average energy is 5.0 (>= 4)
        val highEnergyLogs = listOf(
            EnergyLog(date = "2026-08-20", checkpoint = Checkpoint.MORNING, energy = 5)
        )
        val repoHigh = createGrowthTestRepository(habits = listOf(habit), energyLogs = highEnergyLogs)
        val snapshotHigh = GrowthEngine.evaluateWeekly(plan, repoHigh, today)
        assertEquals(UpgradeDecision.UPGRADE, snapshotHigh.decision)
    }

    /* ---------------------------------------------------- Repository helper */

    private fun createGrowthTestRepository(
        clock: SuperFlowClock = FixedClock(Instant.parse("2026-08-20T10:00:00Z")),
        ...
        val dbProxy = Proxy.newProxyInstance(
            ...
            if (method.name == "query") {
                ...
                val cursor = when {
                    ...
                    sql.contains("FROM checkin") -> createCheckInCursor(checkIns)
        ...
        lockField.set(repo, ReentrantLock())
    ...
    private fun createHabitCursor(habits: List<Habit>): Cursor {
        val rows = habits.map { h ->
            mapOf(
                ...
                "cuePlace" to h.cuePlace, "anchorHabitId" to h.anchorHabitId,
                "anchorText" to h.anchorText, "benefit" to h.benefit,
                "temptationBundle" to h.temptationBundle, "reframe" to h.reframe,
    ...
    private fun createCheckInCursor(checkIns: List<CheckIn>): Cursor {
        val rows = checkIns.map { c ->
            mapOf(
                "id" to c.id, "habitId" to c.habitId, "date" to c.date,
                "result" to c.result.name, "level" to c.level.name,
                ...
                "contextTags" to "", "missReason" to c.missReason,
                "missReasonDetail" to c.missReasonDetail, "createdAt" to c.createdAt
            )
        }
        return createMapCursor(rows)
    }
